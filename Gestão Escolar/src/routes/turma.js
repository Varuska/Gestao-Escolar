...
router.post('/turma', async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Turma feita com sucesso', turma })

    } catch (err) {

        res.status(400).json({ message: 'Erro ao criar nova Turma', err })
        return
...
router.put('/turma/:turmaNumber', async (req, res) => {
    ...
    const { codigoProfessor, capacity } = req.body
